...
| F10-S10 | Code-split real das libs pesadas (recharts/xyflow/fullcalendar) via lazyClient            | 🟢 available   | medium     | —          |
| F10-S11 | Bump de dependências vulneráveis (OWASP A06) — drizzle-orm, OTel, vitest                  | 🔵 in-progress | high       | —          |
| F10-S12 | a11y das telas flagship — ChatList (setas), Pipeline (dnd-kit keyboard), ReactFlow canvas | ⏸️ blocked    | medium     | F10-S10    |
| F10-S13 | Sentry browser — error tracking do cliente web (opt-in, no-op sem DSN)                    | 🟢 available   | medium     | —          |
