...
| F2-S22 | Date em SQL cru quebrava rollup de métricas, cobrança PIX e agendadores — correção no cliente do banco | 🟣 review | critical   | F2-S13                 |
...
| F25-S10 | Tentativa negada à camada de plataforma podia não ficar registrada — auditoria antes da resposta | 🔵 in-progress | high       | F25-S01                   |
...
| ID      | Titulo                                                                   | Status | Prioridade | Depende de |
| ------- | ------------------------------------------------------------------------ | ------ | ---------- | ---------- |
| F46-S01 | Token expirado → purga caches + desconecta socket + redireciona p/ login | ✅ done | high       | —          |
...
| ID      | Titulo                                                                 | Status | Prioridade | Depende de |
| ------- | ---------------------------------------------------------------------- | ------ | ---------- | ---------- |
| F49-S01 | Delay de envio não-bloqueante no nó de mensagem (WAITING + compat)     | ✅ done | high       | —          |
| F49-S02 | Inspector do nó de mensagem — campo de delay + teto de 30s na pré-ação | ✅ done | high       | F49-S01    |
...
| ID      | Titulo                                                                | Status | Prioridade | Depende de       |
| ------- | --------------------------------------------------------------------- | ------ | ---------- | ---------------- |
| F51-S01 | Evento socket flow_execution:updated (@hm/shared)                     | ✅ done | high       | —                |
| F51-S02 | FlowEventsPort + emissão de estado no dispatcher (@hm/flow-engine)    | ✅ done | high       | —                |
| F51-S03 | Publisher real do FlowEventsPort → socket relay (apps/workers)        | ✅ done | high       | F51-S01, F51-S02 |
| F51-S04 | Enriquecer GET executions com flowName + emitir no cancel (apps/api)  | ✅ done | high       | F51-S01          |
...
| ID      | Titulo                                                                                     | Status | Prioridade | Depende de                |
| ------- | ------------------------------------------------------------------------------------------ | ------ | ---------- | ------------------------- |
| F52-S01 | Schema foundation da sincronização (media_status, provider_timestamp, idempotency_key)     | ✅ done | critical   | F40-S01                   |
...
| F52-S04 | Outbound confiável — idempotência de envio + race do callback de status                    | ✅ done | high       | F52-S01                   |
| F52-S05 | Resiliência de mídia — media_status, retry de download, evento media_failed                | ✅ done | high       | F52-S01                   |
| F52-S06 | Endpoint de refresh de signed URL de mídia expirada                                        | ✅ done | medium     | —                         |
| F52-S07 | Frontend realtime — resync ao reconectar + listener de status + UI de mídia (erro/retry)   | ✅ done | high       | F52-S04, F52-S05, F52-S06 |
| F52-S08 | Inbound consistente — ordenação fiel (provider_timestamp) + dedup de disparo de flow       | ✅ done | high       | F52-S01                   |
| F52-S09 | Observabilidade da sincronização — métricas de tick, profundidade de fila, status WhatsApp | ✅ done | medium     | —                         |
| F52-S10 | Lock distribuído Redis para outbound (ordem multi-instância) + tuning                      | ✅ done | medium     | F52-S04                   |
...
| F53-S03 | Modal de agendamento rápido + atalhos de data                                         | ✅ done | high       | F53-S02          |
| F53-S04 | Card Agenda + Histórico no Cockpit                                                    | ✅ done | high       | F53-S03          |
...
| ID      | Titulo                                                                                    | Status | Prioridade | Depende de                |
| ------- | ----------------------------------------------------------------------------------------- | ------ | ---------- | ------------------------- |
| F55-S01 | Timestamps de ciclo em conversations (first_response_at/resolved_at/closed_at) + backfill | ✅ done | critical   | —                         |
| F55-S02 | Popular first_response_at/resolved_at/closed_at nas transições reais (write path)         | ✅ done | high       | F55-S01                   |
| F55-S03 | Recriar mv_dashboard_daily_30d sobre resolved_at (não updated_at)                         | ✅ done | high       | F55-S01                   |
...
| F55-S09 | QA ponta a ponta da F55 — role-awareness, dados, realtime, design                         | ✅ done | high       | F55-S06, F55-S07, F55-S08 |
...
| F56-S08 | RLS como backstop real — FORCE RLS + role não-superuser + agent_templates       | ✅ done | high       | —          |
| F56-S09 | Uploads — bloquear SVG + validar magic bytes                                    | ✅ done | medium     | —          |
| F56-S10 | AbacatePay — HMAC obrigatória em produção (secret fora da query)                | ✅ done | medium     | —          |
| F56-S11 | Agente IA — anti-prompt-injection + moderação de entrada/saída                  | ✅ done | high       | —          |
...
| F56-S17 | Workers ops — healthcheck + graceful drain + DLQ alert + lock watchdog          | ✅ done | high       | F56-S25    |
...
| F56-S26 | DS — promover EmptyState/Skeleton/ErrorState para @hm/ui                        | ✅ done | medium     | —          |
| F56-S27 | DS — Drawer canônico + IconButton (foco de teclado)                             | ✅ done | medium     | F56-S26    |
| F56-S28 | DS — tokenizar e aplicar a escala tipográfica editorial                         | ✅ done | medium     | —          |
